PROGRAM genENM
   ...
   forceres,ukn,usesec,qexist,lig1,firstat, custb, dna, hin, is_numeric, pf 
   ...
   distave,drms,kij,kset,shift,rx,ry,rz,trace,random,masstol,xav,yav,zav,bfacav,entot,back_enhance, &
   spring_radius
   ...
   IF (getoption('-pf',.false.,dummy)) THEN
      pf=.true.
   ELSE
      pf=.false.
   END IF
   ...
   IF (getoption('-c',.true.,dummy)) THEN
      ...
      IF (getoption('-b',.true.,dummy)) THEN
         IF (is_numeric(dummy(1:1))) THEN
            READ(dummy,*) back_enhance
         ELSE
            STOP "The backbone enhancement coefficnt must be grater than one."
         END IF 
      ELSE 
         back_enhance = 1
      END IF
         ...
         DO i=1,natom
            ...
            IF (cutvect) THEN
               ...
               DO
                  ...
                  OPEN(file='draw_enm.pml',form='FORMATTED',unit=7433)
                  ...
                  DO i=1,natom
                     ...
                     DO j=1,natom
                        IF (i.ne.j) THEN
                           ...
                           IF (hin) THEN
                              ...
                              ELSEIF (rkh.gt.0.d0) THEN
                                 ...
                                 IF (pf.or.(dist.le.cutoff)) THEN
                                    ! Parameter free ENM (pfENM)
                                    IF (pf) kij=kij/dist2
                        ...
                        ! Close PyMOL file
                        WRITE(7433,'(A)') 'cmd.set_bond("stick_color", "black", "CAonly")'
                        WRITE(7433,'(A)') 'cmd.show_as("sticks", "CAonly")'
                        WRITE(7433,'(A)') 'cmd.show("spheres", "CAonly")'
                        WRITE(7433,'(A)') 'cmd.set("sphere_scale", 0.8, "CAonly")'
                        WRITE(7433,'(A)') 'cmd.bg_color("grey80")'
                        WRITE(7433,'(A)') 'cmd.orient("CAonly")'
                        CLOSE(7433)
                     ...
                     SUBROUTINE helptext(iunit)
                        ...
                        WRITE(iunit,'(A)')"                    [-fcust ffile] [-spcust spfile] [-mass] [-ca] [-pf]  "
                        WRITE(iunit,'(A)')"                    [-lig1] [-res] [-het] [-hin] [-hine h] [-an p]       " 
                        WRITE(iunit,'(A)')"                    [-b back]                                            " 
                        WRITE(iunit,'(A)')"                                                                         "
                        ...
                        WRITE(iunit,'(A)')" -pf       Opt                   Calculates parameter free ENM           "
                        ...
                        WRITE(iunit,'(A)')"  -b       Input,Opt             Strengthens CA backbone atoms by a      "
                        WRITE(iunit,'(A)')"                                 factor of back. BENM implementation.    "
                        WRITE(iunit,'(A)')"                                                                         "
                        WRITE(iunit,'(A)')"                                                                         "
